...
const TemplatesGallery = dynamic(
  () =>
    import("../../components/flow/controls/TemplatesGallery").then(
...
const CodePreview = dynamic(
  () =>
    import("../../components/flow/controls/CodePreview").then(
      (m) => m.CodePreview,
    ),
  { ssr: false },
);
const LivePreview = dynamic(
  () =>
    import("../../components/flow/controls/LivePreview").then(
      (m) => m.LivePreview,
    ),
  { ssr: false },
);
const DeployLogs = dynamic(
  () =>
    import("../../components/flow/controls/DeployLogs").then(
      (m) => m.DeployLogs,
    ),
  { ssr: false },
);
const CloudSettingsModal = dynamic(
  () =>
    import("../../components/CloudSettingsModal").then(
      (m) => m.CloudSettingsModal,
    ),
  { ssr: false },
);
  ...
  useEffect(() => {
    ...
    const loadPromise = projectId
      ? fetch(`/api/projects/${projectId}`)
          .then((res) => res.json())
          .then((data) => {
            const graphData = data?.project?.graph_data;
            if (
              graphData &&
              Array.isArray(graphData.nodes) &&
              Array.isArray(graphData.edges)
            ) {
              useGraphStore.getState().loadGraph(graphData);
            } else {
              load();
            }
          })
