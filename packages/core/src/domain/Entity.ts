...
export class Entity implements Named, Validatable<Entity>, Configurable {
  ...
  readonly options: Required<Omit<EntityOptions, "description">> &
    Pick<EntityOptions, "description">;
  ...
  validate(): ValidationResult<Entity> {
    ...
    if (!/^[A-Z][a-zA-Z0-9]*$/.test(this.name)) {
      errors.push({
        path: "name",
        message: "O nome da entidade deve estar em PascalCase",
    ...
    if (this.fields.length === 0) {
      errors.push({
        path: "fields",
        message: "A entidade deve ter pelo menos um campo",
    ...
    for (const field of this.fields) {
      if (fieldNames.has(field.name)) {
        errors.push({
          ...
          message: `Campo duplicado: ${field.name}`,
    ...
    if (
      this.hasFeature(FeatureFlag.AUDIT_LOG) &&
      this.hasFeature(FeatureFlag.SOFT_DELETE)
    ) {
      const hasDeletedAt = this.fields.some((f) => f.name === "deletedAt");
      if (!hasDeletedAt) {
        errors.push({
          path: "features",
          message: "Exclusão suave requer um campo deletedAt",
