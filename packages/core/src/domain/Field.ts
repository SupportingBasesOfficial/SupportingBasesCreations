...
export class Field implements Named, Validatable<Field>, Configurable {
  ...
  validate(): ValidationResult<Field> {
    ...
    if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(this.name)) {
      errors.push({
        path: "name",
        message: "Nome do campo deve ser um identificador válido",
        code: "FIELD_NAME_INVALID",
      });
    }

    if (this.type === FieldType.ENUM) {
      const enumValues = (this.options as Record<string, unknown>)
        .enumValues as string[] | undefined;
      if (!enumValues || enumValues.length === 0) {
        errors.push({
          path: "options.enumValues",
          message: "Campo enum deve ter valores",
    ...
    if (this.type === FieldType.RELATION) {
      if (!this.options.targetEntity) {
        errors.push({
          ...
          message: "Campo de relação deve especificar a entidade alvo",
      ...
      if (!this.options.relationType) {
        errors.push({
          ...
          message: "Campo de relação deve especificar o tipo de relação",
    ...
    for (const validator of this.options.validators ?? []) {
      ...
      if (!result.success) {
        errors.push({
          path: "validators",
          message: "Validador personalizado falhou",
