...
describe("CLI Integration", () => {
  ...
  it("should generate a project with default template", () => {
    ...
    const out = execSync(
      `node ${CLI_PATH} generate -n test-app -o ${outputDir} --dry-run`,
      { encoding: "utf-8" },
    );
    expect(out).toContain("Dry-run summary");
  });

  it("should auto-fix invalid project names", () => {
    const outputDir = join(tempDir, "autofix-project");
    const out = execSync(
      `node ${CLI_PATH} generate -n "Bad Name" -o ${outputDir} --dry-run`,
      { encoding: "utf-8" },
    );
  ...
  it("should reject empty project names", () => {
    const outputDir = join(tempDir, "empty-name-project");
    expect(() => {
      execSync(`node ${CLI_PATH} generate -n "" -o ${outputDir}`, {
        encoding: "utf-8",
      });
    }).toThrow();
  });
